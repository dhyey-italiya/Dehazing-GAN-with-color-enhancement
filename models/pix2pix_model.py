...
class Pix2PixModel(BaseModel):
    ...
    def backward_G(self):
        ...
        #color loss
        self.color_loss = color_loss(self.real_B, self.fake_B)
        #perceptual loss
        if self.opt.vgg > 0:
            self.loss_vgg = self.vgg_loss.compute_vgg_loss(self.vgg, self.real_B, self.fake_B) * self.opt.vgg if self.opt.vgg > 0 else 0
        # combine loss and calculate gradients
        self.loss_G = self.loss_G_GAN + self.loss_G_L1 + 0.05*self.color_loss + self.loss_vgg
